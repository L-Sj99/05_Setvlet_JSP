...
<body>
  ...
  <pre>
  (간단한 정의)
  - JSP에서 Javs 코드 값을 쉽게 출력하는 언어 표기법
  - \${key} 형태로 작성
  </pre>
  <form action="/el/test1">
  <%-- form 태그 method 기본 값은 GET --%>
  ...
  <h3>forward : 요청 위임</h3>
  <pre>
    - 클라리언트의 요청을 받은 Servlet/JSP가 직접 응답 하는 것이 아닌 다른 Servlet/JSP에
      HttpServletRquest, HttpServletResponse 객체를 넘겨서 (위임) 대신 응답하게 하는 것
  ...
  <pre>
    ...
    - 보통 Servlet 요청 처리 후 특정 JSP로 요청을 위임해 결과 화면을 응답해 주는 것이 아닌
      ...
      다른 Servlet을 요청하여 다른 Servlet의 결과 화면을 응답해줌
    - redirect는 다시 요청하는 것
      -> 기존 client의 request, response 객체가 사라지고 새로운 "request, response 객체가 생성"된다
        -> request scope 범위에 셋팅된 값들이 모두 사라짐
    - redirect는 응답 화면의 주소가 처음 요청한 Servlet 주소가 아닌 다시 요청한 Servlet 주소로 변경됨
    - 다른 Servlet을 요청하기 위해서는 "요청 주소"를 작성해야 한다
  </pre>
  <form action="/fr/redirect">
    redirect 입력 1 : <input type="text" name="redirectdInput1">
    <br>
    redirect 입력 2 : <input type="text" name="fredirectInput2">
