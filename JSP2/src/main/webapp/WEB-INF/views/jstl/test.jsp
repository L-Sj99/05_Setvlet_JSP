...
<body>
  ...
  <pre>
    - 원하는 scope(범위) 객체에 값을 셋팅할 수 있는 태그
      == 객체.setAttribute("key", value);

    [속성]
    1) var : 변수명(==key)
    2) value : 대입할 값(value)
    3) scope : 범위지정(page, request, session, application) -> 기본 값은 page
  </pre>
  ...
  <h5>application - num4 : ${num4}</h5>
  <hr>

  <h3>2. 변수 제거(== 속성 제거) - c:remove 태그</h3>
  <pre>
    - scope 객체에 셋팅된 속성을 제거하는 태그(== 객체.removeAttribute("key"))

    [속성]
    1) var : 변수명(== key)
    2) scope : 범위지정(page, request, session, application) -> 기본 값 : 전 범위 일괄 제거
  </pre>
...
<pre>
  ...
  * 주의사항 : else 구문이 없음
  ...
  [속성]
  test : 조건식을 작성하는 속성
  - 작성법1 : 대입되는 값은 무조건 EL 구문으로 작성
  - 작성법2 : 작성된 조건식의 결과는 무조건 T/F
</pre>
...
<c:if test='${test3 != "숯불 닭갈비"}' >
  ...
  <pre>
    [c:choose 태그]
    - 내부에다가 조건문 태그를 작성하겠다는 태그 : 내부에다가 c:when, c:otherwise 태그만 작성 가능

    * 주의사항 *
    c:choose태그 바로 한단계 아래에는 c:when, c:otherwise, 공백, jsp 주석만
    작성할 수 있고 나머진 모두 에러 발생
  ...
  <h4>파라미터(쿼리스트링)로 전달된 값에 따라 동작하기</h4>
  <c:choose>
    <%-- 파라미터 중 age가 없을 경우 --%>
    ...
    <h4 style="color:blue">age 값이 없습니다</h4>
    </c:when>
