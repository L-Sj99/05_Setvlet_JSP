...
<body>
  <h2>EL의 특징</h2>
    <pre>
    1. EL은 null을 빈칸("")으로 처리 + NullPointerException을 발생시키지 않음
    2. get 이라는 단어를 거의 사용하지 않는다 : EL은 표현(출력)언어
      -> 값을 대입하고, 조건/반복하는 구문을 작성x, 오로지 출력
  ...
  - \${paramValuse.key} : key 일치하는 파라미터 모두 얻어오기
  </pre>
  ...
  <h3>EL은 자료형 변환(parsing)을 자동으로 수행해준다</h3>

  <ul>
    <%-- EL은 Java로 변환 되지면 문자열 비교는 == (비교 연산자) 사용 가능 --%>
    <li>${param.str == "abc"}</li>
    <%-- HTML에서 얻어온 데이터는 모두 String. 하지만 EL에서 연산되는 자료형이 다를 경우 자동으로 자료형 변환(Parsing)을 진행해준다  --%>
  ...
  <ul>
    ...
    <%-- check라는 key를 가진 모든 값을 모다 string[]로 반환 --%>
    <li>
