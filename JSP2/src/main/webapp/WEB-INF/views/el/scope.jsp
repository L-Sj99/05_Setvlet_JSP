...
<h1>Servlet/JSP 범위별 내장 객체</h1>
  ...
  <h3>requst(요청) == HttpServletRequest</h3>
  ...
  <h3>3. session (입회, 접속)</h3>
    ...
    - [중요!] session은 브라우저 마다 하나씩 생성(새탭, 새창은 아님)

    - [유지 범위]
      사이트 접속 ~ 브라우저 종료 | 세션 만료

    - session이 유지되는 상태에서는 아무 곳에서나 가져다 사용할 수 있음
  ...
  <h3>범위별 객체에 셋팅된 값(속성) 얻어오기</h3>
  <pre>
  - 제출된 파라미터 얻어오는 EL : \${param.key}
  - 범위별 객체에 셋팅된 값 얻어오는 EL
  1) \${oooScope.key} ex.\${requestScope.key}
  2) \${key} : 좁은 범위 객체부터 탐색하여 일치하는 key가 있으면 얻어옴
  </pre>
  <ul>
  <li>page scope : </li>
...
<h1>범위별 객체 우선순위 확인</h1>
  <% // page scope 객체에 값 세팅
    pageContext.setAttribute("menu", "돈가스(page)");
  %>
  <h3>menu : ${menu}</h3>
  <hr>

  <h3>원하는 scope의 셋팅된 값 얻어오기</h3>
  <sl>
    <li>pageScope.menu : ${pageScope.menu}</li>
    <li>resuestScope.menu : ${requestScope.menu}</li>
