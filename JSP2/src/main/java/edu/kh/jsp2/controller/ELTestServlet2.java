...
public class ELTestServlet2 extends HttpServlet{
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		// 2. request scope : 요청 받은 Servlet/JSP와 요청이 위임된 Servlet/JSP에서 유지되는 객체
		
		// *Servlet/JSP 범위 객체에 추가적인 값(속성) 추가하는 방법
		// 1. 값(속성) 추가 방법 : 범위 객체.setAttribute("key", value);
		// 2. 값(속성) 얻어오는 방법 : Object 범위 객체.getAttribute("key");
		...
		// 3. session(입회, 접속) scope : 클라이언트가 서버에 첫 요청 시 서버쪽애 생성되는 객체
		...
		// 5. 범위별 우선순위 확인(좁은 범위가 우선순위가 높다)
		// page > request > session > application
		// key 값을 동일하게 하여 범위별 객체에 값을 추가
		req.setAttribute("menu", "우육면(request)");
		session.setAttribute("menu", "칼국수(session)");
		application.setAttribute("menu", "우동(application)");
