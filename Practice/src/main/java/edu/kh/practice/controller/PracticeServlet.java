...
public class PracticeServlet extends HttpServlet{
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if(index > -1) {
			...
			// JSP 파일 경로를 wabapp 폴더 기준으로 작성 : WEB-INF 폴더는 클라이언트가 직접 접근 불가 하지만 서버는 가능
			String path = "/WEB-INF/views/result.jsp";
			
			// 지정된 경로의 jsp에서 응답을 대신하라고 위임. 이 때, 기존 req,resp가 유지됨
			...
			// message를 session scope에 셋팅한 이유 : redirect(재요청)시 기존 req/resp 객체가 사라지고 새 req/resp 객체가 생성되기 때문에
			// 기본 req에 값을 셋팅 해봐야 없어지기 때문에 의미 없음
